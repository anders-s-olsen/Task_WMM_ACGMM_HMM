        ...
        for r in range(num_repsouter):
            ...
            for r2 in range(num_repsinner):
                if m==0:
                    ...
                    model0 = TorchMixtureModel(distribution_object=ACG,K=K, dist_dim=num_regions,regu=1e-5)
                    ...
                    like = train_hmm_batch(model0, data=data_train, optimizer=optimizer, num_epoch=int_epoch, keep_bar=False,early_stopping=False)
                    model = model0
                elif m==1:
                    model1 = HMM(num_states=K, observation_dim=num_regions, emission_dist=ACG,regu=1e-5)
                    optimizer = optim.Adam(model1.parameters(), lr=0.1)
                    like,model1,like_best = train_hmm(model1, data=data_train, optimizer=optimizer, num_epoch=int_epoch, keep_bar=False,early_stopping=False)
                    model = model1
                elif m==2:
                    model2 = TorchMixtureModel(distribution_object=Watson,K=K, dist_dim=num_regions)
                    optimizer = optim.Adam(model2.parameters(), lr=0.01)
                    like,model2,like_best = train_hmm(model2, data=data_train_concat, optimizer=optimizer, num_epoch=int_epoch, keep_bar=False,early_stopping=False)
                    model = model2
                elif m==3:
                    model3 = HMM(num_states=K, observation_dim=num_regions, emission_dist=Watson)
                    optimizer = optim.Adam(model3.parameters(), lr=0.01)
                    like,model3,like_best = train_hmm(model3, data=data_train, optimizer=optimizer, num_epoch=int_epoch, keep_bar=False,early_stopping=False)
                    model = model3
