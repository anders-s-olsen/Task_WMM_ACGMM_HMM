...
from src.models.AngularCentralGauss_init import AngularCentralGaussian as ACG_init
    ...
    for r in range(num_repsouter):
        for m in range(4):
            ...
            for r2 in range(num_repsinner):
                ...
                init['mu'] = torch.zeros((K,data_train.shape[2]))
                init['kappa'] = torch.zeros(K)
                for k in range(K):
                    init['mu'][k] = param['mix_comp_'+str(k)]['mu']
                    init['kappa'][k] = param['mix_comp_'+str(k)]['kappa']

                # train ACG MM on Watson init
                model0 = TorchMixtureModel(distribution_object=ACG_init,K=K, dist_dim=data_train.shape[2],regu=1e-05,init=init)
                ...
                for kk in range(K):
                    ...
                    plt.figure(),plt.imshow(torch.outer(init['mu'][kk],init['mu'][kk])),plt.colorbar()
                ...
                if m==0:
                    ...
                    post = model0.posterior(data_test_concat.to(device)).cpu()
